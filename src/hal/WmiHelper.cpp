...
std::vector<std::wstring> WmiHelper::GetInstancePaths(const std::wstring &className) {
  std::vector<std::wstring> paths;
  if (!m_initialized)
    return paths;

  std::wstring query = L"SELECT __RELPATH FROM " + className;
  BSTR lang = SysAllocString(L"WQL");
  BSTR q = SysAllocString(query.c_str());
  IEnumWbemClassObject *pEnum = nullptr;
  HRESULT hr = m_pSvc->ExecQuery(lang, q, WBEM_FLAG_FORWARD_ONLY, nullptr, &pEnum);
  SysFreeString(lang);
  SysFreeString(q);
  if (FAILED(hr))
    return paths;
  ...
  ULONG ret = 0;
  while (pEnum && pEnum->Next(WBEM_INFINITE, 1, &obj, &ret) == S_OK && ret > 0) {
    VARIANT vt;
    VariantInit(&vt);
    if (SUCCEEDED(obj->Get(L"__RELPATH", 0, &vt, nullptr, nullptr)) &&
        vt.vt == VT_BSTR && vt.bstrVal) {
      paths.push_back(vt.bstrVal);
    }
    VariantClear(&vt);
    obj->Release();
  }
...
bool WmiHelper::GetUint32Property(const std::wstring &relPath,
                                  const std::wstring &propName, uint32_t &out) {
  if (!m_initialized)
    return false;
  BSTR path = SysAllocString(relPath.c_str());
  IWbemClassObject *obj = nullptr;
  HRESULT hr = m_pSvc->GetObject(path, 0, nullptr, &obj, nullptr);
  SysFreeString(path);
  if (FAILED(hr))
    return false;
  VARIANT vt;
  VariantInit(&vt);
  bool ok = SUCCEEDED(obj->Get(propName.c_str(), 0, &vt, nullptr, nullptr)) &&
            (vt.vt == VT_UI4 || vt.vt == VT_I4);
  if (ok)
    out = (vt.vt == VT_UI4) ? vt.ulVal : (uint32_t)vt.lVal;
  VariantClear(&vt);
  obj->Release();
  return ok;
}

bool WmiHelper::PutUint32Property(const std::wstring &relPath,
  ...
  vt.vt = VT_UI4;
  vt.ulVal = value;
  bool ok = SUCCEEDED(obj->Put(propName.c_str(), 0, &vt, 0));
  if (ok)
    ok = SUCCEEDED(m_pSvc->PutInstance(obj, 0, nullptr, nullptr));
  obj->Release();
  return ok;
}
